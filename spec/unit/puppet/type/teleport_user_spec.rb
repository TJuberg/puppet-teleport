...
describe Puppet::Type.type(:teleport_user) do
  it 'should require a name' do
    expect Puppet::Type.type(:teleport_user).new({}).to raise_error(Puppet::Error, 'Title or name must be provided')
  end
  ...
  context 'when creating a user' do
    ...
    it 'should accept an array of allowed logins' do
      @user[:allowed_logins] = %w[testing root]
      expect(@user[:allowed_logins]).to eq(%w[testing root])
    end
  end
end
